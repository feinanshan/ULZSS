    ...
    def output_inst_pred(self, fea_i, lbl_i, embds, ignr_idx=250):
        ...
        for idx_in in unique_class:
            if idx_in.cpu().item() is not ignr_idx:
                ...
                l_mu = self.decoder.pred_mu(pred_fea)
                ...
                n_,c_,h_,w_ = l_mu.size()

                proto = torch.randn(n_,5,h_,w_).cuda()
                l_mu_ = l_mu.expand_as(proto)
                l_delta = l_delta.expand_as(proto)
     
                l_mu_ = l_mu_+l_delta*proto
                l_mu_ = self.decoder.sigmoid(l_mu_)
                l_mu_ = l_mu_.mean(dim=1).view(n_,c_,h_,w_)

                l_mu = self.decoder.sigmoid(l_mu)
               

                if self.training:
                    loss_g = self.dice_loss(l_mu, idx_mask)/(g_delta)+torch.log(g_delta.sqrt())
                    loss_l = self.loss_fn(l_mu_, idx_mask).mean()#- 0.5*torch.log(l_delta.sqrt()).mean()
